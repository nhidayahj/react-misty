...
export default function Cart() {
    ...
    const fetch = async () => {
      ...
      if (localStorage.getItem("customer_id") !== null) {
        ...
        let items = await axios.get(
          baseUrl + `/api/shoppingCart/${localStorage.getItem("customer_id")}`
        );
  ...
  const incrementDiffQty = (e) => {
    let itemIndex = diffuserItem.findIndex(
      (d) => d.diffusers.id === parseInt(e.target.name)
    );
  ...
  const updateDiffQty = async (e) => {
    ...
    if (updateQty.status === 200) {
      alert("Diffuser quantity updated");
  ...
  const removeDiff = async (e) => {
    ...
    let removeDiff = await axios.get(
      `${baseUrl}/api/shoppingCart/diffuser/${customer_id}/${e.target.name}/remove`
    );
  ...
  const incrementOilQty = (e) => {
    let itemIndex = oilItem.findIndex(
      (i) => i.oils.id === parseInt(e.target.name)
    );
    let cloned = [...oilItem];
    cloned[itemIndex].quantity += 1;
    setOil(cloned);
  ...
  const removeOil = async (e) => {
    ...
    let removeOil = await axios.get(
      `${baseUrl}/api/shoppingCart/oil/${customer_id}/${e.target.name}/remove`
    );
        ...
        <React.Fragment>
          <div className="container mt-3 mb-3">
            <ListGroup className={`${d.diffusers.id}`}>
              <img
                src={d.diffusers.image_url}
                className="cart-item-img"
                style={{ width: "175px", height: "197px" }}
                name={d.diffusers.id}
              />
              <ListGroupItem
                className="cart-item-name mt-3"
                id={d.diffuser_id}
                name={d.diffusers.id}
              >
                Item: {d.diffusers.diffuser_name}
              </ListGroupItem>
              <ListGroupItem id={d.diffuser_id}>
                <div className="cart-qty">Quantity: {d.quantity} </div>
              ...
              <ListGroupItem
                ...
                name={d.diffusers.id}
              >
                Price: {(formatPrice(d.diffusers.cost) * d.quantity).toFixed(2)}{" "}
                SGD
              </ListGroupItem>
            </ListGroup>
          </div>
    ...
    for (let i of oilItem) {
      oilCart.push(
        <ListGroup className={"mt-3 mb-3" + " " + `${i.oils.id}`}>
          ...
          <ListGroupItem className="cart-price" id={i.oil_id}>
            Price: {(formatPrice(i.oils.cost) * i.quantity).toFixed(2)} SGD
          </ListGroupItem>
        </ListGroup>
      );
    }
  ...
  const confirmOrder = () => {
    if (diffuserItem.length < 1 || oilItem.length < 1) {
      alert(
        "Must have at least 1 diffuser and 1 essential oil before checkout"
      );
    } else {
  ...
  function displayCheckoutBtn() {
    return (
      <React.Fragment>
        <h3 className="checkout-amt-text">
          Total Amount: {displayFinalAmt()} SGD
        </h3>
        <Button
          className="mt-4 mb-4 confirm-btn"
          color="info"
