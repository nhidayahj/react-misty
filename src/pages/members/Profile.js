...
export default function MemberProfile() {
  ...
  const goCatalog = () => {
    history.push(`/diffusers`);
  };

  const updateProfile = async (e) => {
    try {
      let updateInfo = await axios.post(
        `${baseUrl}/api/members/info/${e.target.name}/update`,
        {
          name: profile.name,
          email: profile.email,
  ...
  return (
    <React.Fragment>
      <div className="container profile-box">
        ...
        <Button
          color="info"
          className="profile-update mt-3"
          onClick={updateProfile}
          name={localStorage.getItem("customer_id")}
        >
          Update Profile
        </Button>{" "}
        <Button color="success" className="mt-3 profile-cart" onClick={goCart}>
          Shopping Cart
        </Button>{" "}
        <Button color="secondary" className="mt-3 catalog" onClick={goCatalog}>
          Browse Catalog
        </Button>
      </div>
